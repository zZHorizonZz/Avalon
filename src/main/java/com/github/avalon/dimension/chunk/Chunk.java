 ...
 * com.github.avalon.dimension.DimensionManager} to manage {@link Dimension} data. Chunk is
 * 16x16 segment of blocks. From which is created whole world.
 *
 * @author Horizon
 * @version 1.0
 */
public interface Chunk extends NamedBinarySerializer {

  int CHUNK_SECTIONS = 16;
  int CHUNK_SECTION_SIZE = 16;

  ChunkBatch getChunkBatch();
  ...
  /**
   * {@link ChunkSection} should not be edit otherwise client can crash or something else can
   * happen. Everytime we want to edit these sections we should unload this chunk and then load it
   * again.
   *
   * @return Chunk sections of the current chunk.
   */
